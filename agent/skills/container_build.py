...
# just works when baked. (Build tools bloat the runtime image — a multi-stage
# cleanup is a Phase-D leanness optimization, not a correctness issue.)
...
def emit_dockerfile(
    ...
    *,
    ...
    if has_env_layer:                       # engine present ONLY if conda/pip specs were declared
        lines.append(f'LABEL env_engine="{engine.name}"')
    lines += [
        "",
