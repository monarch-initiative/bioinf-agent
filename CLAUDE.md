...
| `freeze(env, tools, pipeline_name?, platform?, accel?, gated?, push_target?, pipeline_id)` | Freeze the env into a content-addressed, HPC-shippable artifact. request_key→EnvCache lookup (hit = proven artifact by hash, no re-solve). ADOPT-or-BUILD by what the env actually contains: **pure conda + a published BioContainer → ADOPT by digest**; **any non-conda install (binary/jar/source/…) → RECIPE BUILD** that replays the installs on the ship platform (release binaries re-fetched from the SAME release's linux asset, sha256-anchored; JAR tools re-fetched as arch-independent bytecode + a JRE; smoke-tested in-image — a foreign biocontainer is NOT adopted, it would ship a different unvalidated artifact); **pure conda, no biocontainer → conda-pack build only when host arch == ship arch** (a cross-arch conda-pack is refused, not shipped). Replay covers **conda + binary + jar + source + cargo + go** today (source = clone at the pinned commit + rebuild on the ship platform with a build toolchain; needs `build_command` + `bin_path` recorded by install_git_repo. cargo/go = rebuild on the ship platform with the **SAME toolchain version captured at install** — Rust via rustup pinned to the host `rustc`, Go via the official pinned tarball for the ship arch; `GOTOOLCHAIN=local` refuses a silent toolchain swap, so a version mismatch fails loudly rather than shipping a different build. perl = replay `cpanm` on the ship platform; perl+cpanm come from the conda layer (the version pin), and **XS modules compile against the conda perl** via the conda C/C++ toolchain + an xlocale.h shim + env activation — the load-or-die `perl -M{module} -e1` runs IN the build). **All install tiers are now replayable** — an unknown type would still hard-stop with a clear message rather than ship a broken image. Records `shipped_binaries[]` (the ship-platform hash vs the host-validated hash) + `build_method`. Emits the Apptainer HPC delivery contract (registry-free `docker save`→`apptainer build docker-archive` by default; gated ⇒ tarball-only per I13). `tools` = the PRIMARY requested tools |
