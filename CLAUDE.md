...
| `build_env_from_authors_recipe(repo, tools, name, recipe?, ref?, version?, build_args?, gated?, licenses?)` | **The declarative sibling** (route here from resolve_tool's `authors_recipe` tier — taken when the authors' recipe installs deps a conda/pip reconstruction would silently DROP). Clones `repo` at a pinned `ref` (PIN a tag/commit — a bare default branch drifts), `docker build`s its `recipe` (default `Dockerfile`), then hands the image to `freeze_from_image` (contract + deliverables). The recipe records the pinned source so the build is reproducible. Docker + git + network required |
