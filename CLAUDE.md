...
- **Seal-required fields the invariants DON'T catch** (they surface only as a pydantic `WorkflowSpec validation failed` at write time, *after* every invariant passes): `usage.description` (a one-line string — required whenever a `usage` block is present) and `reference_databases[*].source_url` (required per entry; for a locally-staged reference with no download origin, a `file://<abs_path>` URL is acceptable). Fill both before `seal_workflow` to avoid a late refusal.
