...
3. **`select_test_data(...)`** — pick a dataset from `data/core_test_data_hg38/manifest.yaml` (or generate one with `phenopacket_to_vcf` or a small Rscript / Python). It sha256-anchors every path it records (`test_data.content_anchors`) so I8 can re-verify the inputs at seal; `test_data` is not patchable, so this primitive is the only producer of an anchor.
