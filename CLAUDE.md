...
| `upload_to_scratch / download_from_scratch / upload_to_common_data / download_from_common_data / upload_to_project_path / download_from_project_path` | **HPC bridge — three transfer auth families × two wire protocols.** All refuse-to-overwrite + verify integrity. **scratch** (env-implicit + auto-prefix by project — sandbox: per-run staging, logs); **common_data** (env-implicit + shared — reference data, staged `.sif`s; NO project prefix); **project_path** (Phase-1 explicit + literal abs_path — the user's real project layout; longest-prefix match against `directories[]`). Discrete capabilities per primitive: `upload` ≠ `download` ≠ `exec`. Wire protocol selected by `env.data_transfer.type`: `scp_head_node` (default, scp + sha256 round-trip) or `globus` (encrypted, off-head-node, end-to-end checksum). Each primitive accepts `async_globus=True` to return a task_id immediately for huge transfers; poll via `globus_task_status`. |
| `globus_task_status(project, env, task_id)` | **HPC bridge — async-Globus poll.** Pairs with `async_globus=True` on the upload/download primitives. ONE `globus task show <id> --format json` query; returns `status` (ACTIVE / INACTIVE / SUCCEEDED / FAILED), `nice_status`, `bytes_transferred`, `files_transferred`, `fatal_error`. Task_id validated as UUID BEFORE any shell-out. |
...
- **`globus`** — every `upload_to_X` / `download_from_X` + `stage_apptainer_image`'s `.tar` transfer goes through the Globus CLI. Encrypted end-to-end, off-head-node, checksum-verified by Globus itself. Requires `globus login` + Globus Connect Personal installed locally; UUIDs of both endpoints declared in the env's `data_transfer.globus` block. Hard-errors on Globus failure — never silent fallback to scp.
